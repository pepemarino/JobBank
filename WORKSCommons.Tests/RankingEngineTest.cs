...
public class RankingEngineTest
{
    ...
    {
        var mock = new Mock<ILogger<RankingEngine>>();
        ILogger<RankingEngine> logger = mock.Object;

        parser = new JobDescriptionParser();
        rankingEngine = new RankingEngine(logger);
    }
    ...
    public void CalcularRankingTest()
    {
        ...
        var userSkills = "15 years as a Dotnet developer, Documentation, C#, SQL, JavaScript, TypeScript, ASP.NET Core, ASP.NET MVC, Web API, Entity Framework,REST APIs, OAuth 2.0, Event Driven Systems, SOA, Moq, Selenium, SpecFlow, NUnit, integration testing, MSSQL, Oracle, Postgres, MySQL,HTML5, CSS, jQuery, Knockout, React (light), jQuery, LESS, ADO.NET, LINQ, AutoMapper, GitHub Copilot, KendoUI, VB.Net, Web Forms, XAML, XML, JSON, SignalR, WCF, Mentor Junior Engineers,  MSTest, xUnit, Swagger, OpenAPI, Agent Orchestration, SOLID Principles, Design Patterns";
        var sections = parser.GetSections(jobDescriptionReal);

        var jdCanonical = jobDescriptionReal.ToCanonical();

        // Act
        var ranking = rankingEngine.CalcularRanking(sections, userSkills.ToDenormalized());
